...
*/
var tabFilter,menu=[],liIndex,curNav,delMenu,indexUrl,indexName,
...
layui.define(["element","jquery"],function(exports){
    ...
    Tab.prototype.navBar = function(strData){
        ...
        for(var i=0;i<data.length;i++){
        	var iconClass = "menu-icon-"+Math.round(Math.random()*4);
            ...
            if(data[i].children != undefined && data[i].children.length > 0){
                ...
                if(data[i].icon != undefined && data[i].icon != ''){
                    if(data[i].icon.indexOf("icon-") != -1){
                        ...
                        ulHtml += '<i class="layui-icon '+iconClass+'" data-icon="'+data[i].icon+'">'+data[i].icon+'</i>';
                    }
                }
                ...
                for(var j=0;j<data[i].children.length;j++){
                    if(data[i].children[j].target == "_blank"){
                        ulHtml += '<dd><a data-url="'+this.tabConfig.urlRoot+data[i].children[j].href+'" target="'+data[i].children[j].target+'">';
                    }else{
                        ulHtml += '<dd><a data-url="'+this.tabConfig.urlRoot+data[i].children[j].href+'">';
                    }
                ...
                if(data[i].target == "_blank"){
                    ulHtml += '<a data-url="'+this.tabConfig.urlRoot+data[i].href+'" target="'+data[i].target+'">';
                }else{
                    ulHtml += '<a data-url="'+this.tabConfig.urlRoot+data[i].href+'">';
                }
                if(data[i].icon != undefined && data[i].icon != ''){
                    if(data[i].icon.indexOf("icon-") != -1){
                        ulHtml += '<i class="seraph '+data[i].icon+'" data-icon="'+data[i].icon+'"></i>';
            ...
            ulHtml += '</li>';
            if(i==0){
                indexUrl = this.tabConfig.urlRoot+data[i].href;
                indexName = data[i].title;
			}
        }
        return ulHtml;
    }
	...
	Tab.prototype.render = function() {
		//显示左侧菜单
		var _this = this;
		//$(".navBar ul").html('<li class="layui-nav-item layui-this"><a data-url="'+this.tabConfig.urlRoot+'page/main.html"><i class="layui-icon" data-icon=""></i><cite>工作台</cite></a></li>').append(_this.navBar(dataStr)).height($(window).height()-210);
		var navBarHtml = _this.navBar(dataStr);
		$(".navBar ul").html("").append(navBarHtml).height($(window).height()-210);
		$("#index_url").attr("src",indexUrl);
		//$("#cite_name").text(indexName);
